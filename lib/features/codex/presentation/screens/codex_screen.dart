...
class _CodexScreenState extends State<CodexScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<CodexTopic> pool =
        _gemsOnly ? hiddenGems(kCodexTopics) : kCodexTopics;
  ...
  List<Widget> _byCategory(List<CodexTopic> topics, ColorScheme cs) {
    ...
    for (final CodexCategory c in CodexCategory.values) {
      final List<CodexTopic> mine =
          topics.where((t) => t.category == c).toList();
  ...
  Widget _tile(CodexTopic t, ColorScheme cs) => ListTile(
        ...
        onTap: () => Navigator.of(context).push(
          MaterialPageRoute<void>(builder: (_) => CodexTopicScreen(topic: t)),
        ),
      );
}
